...
You receive a recent "topic ledger" covering the last 90 days and a
"durable_event_registry" covering all prior briefs. For each candidate item,
first compare it with both stores. Match the same underlying event using source
URL, source date, entities, action/counterparty, and summary—not topic_key or
headline wording alone.

  1. Is the item's TOPIC or underlying event already in either store?
...
An exact source URL already present in durable_event_registry is the same
coverage unless the candidate contains a clearly identified later development.
An old source republished or rediscovered with no new operative fact is stale,
not a Delta. Bias toward inclusion only when the candidate identifies a
specific plausible new fact; never infer freshness from a new headline alone.
...
/ synthesis prompt v{{synthesis_version}}. Items deduped against 90-day context
